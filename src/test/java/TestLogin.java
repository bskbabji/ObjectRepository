...
/*
	...
	public void TestMultipleWindows() throws InterruptedException, IOException {
		...
		for (String windowHandle : windowHandleSet) {
			...
			GetScreenShot("SecondScreenshot");
			System.out.println(driver.getTitle());
			
			if ((driver.getTitle()).equals("Salesforce: We Bring Companies and Customers Together")) {
				break;
			}
		}
...
/*	@Test(priority = 2)
	public void TestSliderOne() {
		...
		driver.navigate().to(objProperties.getProperty("MouseOperationsURL")); // Actions class and mouse
																						// operations
		
		driver.switchTo().frame(0);
		Actions act = new Actions(driver);
		WebElement Slider = FindElement(Orep.Slider);
		act.dragAndDropBy(Slider, 400, 0).build().perform();

	}

	@Test(priority = 3)
	public void TestSliderTwo() {
		
		driver.navigate().to(objProperties.getProperty("ResizeableURL"));
		driver.switchTo().frame(0);
		Actions act = new Actions(driver);
		WebElement Slider = FindElement(Orep.Resizeable);
		act.dragAndDropBy(Slider, 50, 0).build().perform();

	}

	@Test(priority = 4)
	public void TestRightClick() throws InterruptedException, IOException, AWTException {
		
		driver.navigate().to(objProperties.getProperty("mouseOpeartionRightClick"));    

		Actions act = new Actions(driver);
		WebElement RightClickEle = FindElement(Orep.RightClickButton);
		act.contextClick(RightClickEle).build().perform();
		GetScreenShot("clickedCopy");
		Thread.sleep(100);
		WebElement CopyEle = FindElement(Orep.RightClickButtonCopyOption);
		act.click(CopyEle).build().perform();

		Robot r = new Robot();
		Rectangle rect = new Rectangle(Toolkit.getDefaultToolkit().getScreenSize());
		BufferedImage img = r.createScreenCapture(rect);
		File f = new File(".//Screenshots/alert.png");
	...
	public void HandlingTables() throws InterruptedException {
		...
		Thread.sleep(1000);
		FindElement(Orep.FirstPage).click();
		String text = FindElement(Orep.NumberOfEntries).getText();
		System.out.println(text);

		String numberOfRowsPerPage = text.substring(text.indexOf("to") + 3, text.indexOf("of") - 1);
		int rowCountPerPage = Integer.parseInt(numberOfRowsPerPage);
		System.out.println("rows per page " + rowCountPerPage);

		String TotalEntries = text.substring(text.indexOf("of") + 3, text.indexOf("entries") - 1);
		int totalEntries = Integer.parseInt(TotalEntries);
		System.out.println("total entries " + totalEntries);

		int pageCount;
		if (totalEntries % rowCountPerPage == 0)
			pageCount = totalEntries / rowCountPerPage;
		else
			pageCount = (totalEntries / rowCountPerPage) + 1;
		...
		for (int page = 1; page <= pageCount; page++) {
			for (int i = 1; i <= rowCountPerPage; i++) {

				// String LastName =
				// driver.findElement(By.xpath("//table[@id='example']/tbody/tr["+i+"]/td[3]")).getText();
				String LastName = FindElementWithVariableXPath(Orep.LastNameTableRow, i).getText();

				if (LastName.equals(objProperties.getProperty("LastNameRequired"))) {
					flag = 1;
					System.out.println("The other details are:");
					// String FirstName =
					// driver.findElement(By.xpath("//table[@id='example']/tbody/tr["+i+"]/td[2]")).getText();
					String FirstName = FindElementWithVariableXPath(Orep.FirstNameTableRow, i).getText();
			...
			if (flag == 1)
				...
				FindElement(Orep.NextPage).click();
		}
	}
	*/
